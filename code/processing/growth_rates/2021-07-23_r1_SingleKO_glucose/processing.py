...
* This script drops some points from the mgl KO that are obviously aberrant.
These points are at the beginning of the growth cycle, so the first 6 points are 
dropped.

* This script drops the replicate 1 for the ∆glt KO as there was contamination in 
...
# Clean as per described in Notes
measurements = []
for g, d in measurement.groupby(['strain', 'replicate']):
    if (g[0] == '∆glt') & (g[1] == 1):
        continue
    elif (g[0] == '∆mgl') & (g[1] == 3):
        d = d[d['elapsed_time_hr'] >= 0.6] 
