...
def self_replicator_FPM(params,
    ...
    if 'nutrients' in args.keys():
        # Determine the number of mutants
        if len(params) > 6:
            n_muts = int(len(params[:-1]) /5)
        else:
            n_muts = 1
        if n_muts > 1:
            c_nt = params[-1] 
            M, M_Rb, M_Mb, T_AA, T_AA_star = np.reshape(params[:-1], (n_muts, 5)).T
        else:
            M, M_Rb, M_Mb, T_AA, T_AA_star, c_nt = params

    else:
        if len(params) > 5:
            n_muts = int(len(params) /5)
        else:
            n_muts = 1
        if n_muts > 1: 
            M, M_Rb, M_Mb, T_AA, T_AA_star = np.reshape(params, (n_muts, 5)).T
        else:
    ...
    if 'nutrients' in args.keys():
        ...
        if n_muts > 1:
            dcnt_dt = np.sum(dcnt_dt)
            out = list(np.reshape(out, (5, n_muts)).T.flatten())
            out.append(dcnt_dt)
        else:
            out.append(dcnt_dt) 
    else:
